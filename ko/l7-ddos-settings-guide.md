...
## 1. 배경
* 클라우드 고객, 그룹사 등 2023년(5건), 2024년(2건), 2025년(2건) DDoS 공격에 따른 서비스 장애 발생
* L7 Slow DDoS 공격 등 네트워크 대역폭이 아닌 어플리케이션 계층의 세션을 장시간 점유
* 네트워크 트래픽량이 적어도 웹 서비스의 자원을 고갈함에 따라 장애 유발
* HTTPS 암호화 트래픽 내 정상 요청처럼 유입됨에 따라 탐지가 어렵가 어려움
* 임계치 기반의 Anti-DDoS로 세션 공격을 대응하는데 한계점 존재

## 2. 목적
* 서버, 네트워크, 어플리케이션 등 종합적인 방어 정책 수립
* 세션 기반 공격 유입 시 웹 서버 자원 고갈 완화 필요
* 세션 유지시간과 연결 수 등 적정 수준 관리 및 웹 서비스 가용성 확보

## 3. 보안 대책
* 웹 서버 하드닝 강화
  * KeepAliveTimeout, RequestReadTimeout, client_body_timeout 등 세션 설정을 최적화하여 비정상 연결로 인한 리소스 점유 최소화
* Anti-DDoS 솔루션 국가 기반 선 차단 설정
  * 차단 기준: 서비스 장애 등 홈페이지가 접근 안될 경우
  * 국가 기준: 긴급한 경우 해외 출발지 IP 차단(한국, 일본 제외)
  * 목적지 웹 서버 대상 DDoS 공격 유입 시 서비스 지연 또는 장애 발생 시 국가 차단 활성화(공격 유입 종료 시 원복)
...
| 번호 | 구분 | 항목 | 확인 여부 | 비고 |
| --- | --- | --- | ---- | ---- |
| 1 | 보안 솔루션 | WAF 웹방화벽 구축 및 운영하고 있는지? |  |  |
...
보안 설정 시 웹 서비스 장애 최소화를 위해 환경 고려 필요
